...
def strict_check(data, counters, random_seed, timeout=5):
    ...
    try:
        ...
        responses_answer = responses_answer[:args.m]  # Take the first m after shuffle

        # Check if at least one answer matches the correct answer
        answer = extract_answer("\\boxed{" + answer + "}")
        correct_found = any(compare_ans(res, answer) or res.strip() == answer.strip() for res in responses_answer)
