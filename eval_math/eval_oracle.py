...
def strict_check(data, counters, timeout=5):
    ...
    try:
        for res in responses:
            ...
            round1_answer = extract_answer(data["response_round1"], data_name = args.data_name)
            answer = extract_answer("\\boxed{" + answer+ "}")
            if (compare_ans(response_answer, answer) or response_answer.strip() == answer.strip()) or \
               (compare_ans(round1_answer, answer) or round1_answer.strip() == answer.strip()):
                right_cnt += 1
            cnt += 1
    except TimeoutError:
        cnt += 1
