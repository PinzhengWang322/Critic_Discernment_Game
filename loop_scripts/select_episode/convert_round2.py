...
parser.add_argument('--mask_history', action="store_true")
...
def format_0shot(data):
    ...
    round1_response = data["round1_response"].strip()
    critic = data['prompt'].split("<|start_header_id|>user<|end_header_id|>\n\n")[-1].split("<|eot_id|>")[0]
    round2_respons = data["response"]
...
for data in tqdm(datas): 
    new_data = format_0shot(data)
    if has_repeated_substring_rolling_hash(new_data["conversations"][-1]["value"], 40, 5): continue
    if has_repeated_substring_rolling_hash(new_data["conversations"][-1]["value"], 128, 2): continue
    if new_data["conversations"][-1]["value"].count("boxed") == 1:
