...
def get_unnormalized_answer(text: str) -> str:
    ...
    match = regex.search(r'\\boxed\{((?:[^{}]+|{(?1)})*?)}', text)
    if match:
...
def extract_answer(pred_str, data_name="minerva_math", use_last_number=True):
    ...
    try:
        with timeout(seconds=5):
            try:
                ...
                try:
                    parsed_x1 = parse_latex(x1.lstrip("\\"))
                    parsed_x2 = parse_latex(x2.lstrip("\\"))                
                except (
