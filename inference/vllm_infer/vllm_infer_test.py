...
def worker(gpu_id, prompts_chunk, original_data_chunk, args, return_list):
    ...
    llm = LLM(model=args.model_path, gpu_memory_utilization=0.98,swap_space=64, max_num_seqs=256, enforce_eager=True)
...
def main():
    ...
    original_data = []
    large_prompts = []
    tokenizer_path = "/path/to/Meta-Llama-3.1-8B-Instruct"
    ...
    for line in line_datas:
        ...
        if len(tokenizer(data['prompt'])['input_ids']) > 4000:
    ...
    for gpu_id in range(num_gpus):
        if num_gpus == 7:
            p = mp.Process(target=worker, args=(gpu_id + 1, prompts_chunks[gpu_id], original_data_chunks[gpu_id], args, return_list))
        else:
    ...
    return_list.extend(large_prompts)
